...
- (IBAction)btnExecute_click:(UIButton *)sender {
    ...
    param.hand.MinSize = 100;
    param.hand.MaxSize = 600;
...
- (void)onPostSetParam:(HVC_ERRORCODE)err status:(unsigned char)outStatus
{
    dispatch_async(dispatch_get_main_queue(), ^{
        // 各フラグ参照
        ExecuteFlag = HVC_ACTIV_HAND_DETECTION | HVC_ACTIV_FACE_DETECTION
...
-(void) onPostExecute:(HVC_RES *)result errcode:(HVC_ERRORCODE)err status:(unsigned char)outStatus
{
    ...
    if((err == HVC_NORMAL) && (outStatus == 0)){
        ...
        for(i = 0; i < result.sizeBody; i++){
            ...
            resStr = [resStr stringByAppendingString:[NSString stringWithFormat:@"  [体の検出] : size = %d, ", dt.size]];
        ...
        for(i = 0; i < result.sizeHand; i++){
            ...
            resStr = [resStr stringByAppendingString:[NSString stringWithFormat:@"  [手の検出] : size = %d, ", dt.size]];
            resStr = [resStr stringByAppendingString:[NSString stringWithFormat:@"x = %d, y = %d, ", dt.posX, dt.posY]];
            resStr = [resStr stringByAppendingString:[NSString stringWithFormat:@"conf = %d\n", dt.confidence]];
        }
        ...
        for(i = 0; i < result.sizeFace; i++){
            ...
            if((result.executedFunc & HVC_ACTIV_FACE_DETECTION) != 0){
                resStr = [resStr stringByAppendingString:[NSString stringWithFormat:@"  [顔の検出] : size = %d, ", fd.size]];
            ...
            if((result.executedFunc & HVC_ACTIV_FACE_DIRECTION) != 0){
                resStr = [resStr stringByAppendingString:[NSString stringWithFormat:@"  [顔の向き] : "]];
            ...
            if((result.executedFunc & HVC_ACTIV_AGE_ESTIMATION) != 0){
                ...
                resStr = [resStr stringByAppendingString:[NSString stringWithFormat:@" %d才, 信頼度 = %d％\n", fd.age.age, fd.age.confidence/10]];
            }
            ...
            if((result.executedFunc & HVC_ACTIV_GENDER_ESTIMATION) != 0){
                ...
                resStr = [resStr stringByAppendingString:[NSString stringWithFormat:@"性別 = %@, 信頼度 = %d％　\n", gender, fd.gen.confidence/10]];
            }
            
            // 視線推定
            if((result.executedFunc & HVC_ACTIV_GAZE_ESTIMATION) != 0){
                resStr = [resStr stringByAppendingString:[NSString stringWithFormat:@"  [視線] : "]];
                resStr = [resStr stringByAppendingString:[NSString stringWithFormat:@"左右 = %d, 上下 = %d\n", fd.gaze.gazeLR, fd.gaze.gazeUD]];
            }
            
            // 目つむり推定
            if((result.executedFunc & HVC_ACTIV_BLINK_ESTIMATION) != 0){
                resStr = [resStr stringByAppendingString:[NSString stringWithFormat:@"  [目つむり測定] : "]];
                resStr = [resStr stringByAppendingString:[NSString stringWithFormat:@"目つむり 左 = %d, 目つむり 右 = %d\n", fd.blink.ratioL, fd.blink.ratioR]];
            }
            
            // 表情推定
            if((result.executedFunc & HVC_ACTIV_EXPRESSION_ESTIMATION) != 0){
                resStr = [resStr stringByAppendingString:[NSString stringWithFormat:@"  [表情測定] : "]];
